...
public class AccountsController : AdminControllerBase
{
    ...
    public IActionResult Index()
    {
        ...
        var model = new ManagementListPageViewModel
        {
            ...
            Subtitle = "Quản trị tài khoản và phân quyền mock cho admin, giáo vụ và giáo viên.",
            Breadcrumbs = Breadcrumbs("Tài khoản"),
            PrimaryActionText = "Tạo tài khoản",
            PrimaryActionUrl = "/Admin/Accounts/Create",
            ToolbarNote = "Dữ liệu đang lấy từ MockDataService. // TODO: connect database later",
            SearchPlaceholder = "Tìm theo email, vai trò, phòng ban...",
            SummaryCards =
            [
                new SummaryCardViewModel { Title = "Tài khoản kích hoạt", Value = accounts.Count(x => x.Status == "Đang hoạt động").ToString(), Description = "Có thể đăng nhập demo", Icon = "bi-person-check", AccentClass = "success" },
                new SummaryCardViewModel { Title = "Tài khoản tạm khóa", Value = accounts.Count(x => x.Status != "Đang hoạt động").ToString(), Description = "Hiển thị badge và action phù hợp", Icon = "bi-person-lock", AccentClass = "warning" }
            ],
            Filters =
            [
                new FilterGroupViewModel { Label = "Vai trò", InputId = "role", Options = [new() { Label = "Tất cả", Value = "" }, new() { Label = "Admin", Value = "Admin" }, new() { Label = "Staff", Value = "Staff" }, new() { Label = "Teacher", Value = "Teacher" }] },
            ...
            Table = new TableViewModel
            {
                ...
                Rows = accounts.Select(x => new TableRowViewModel
                {
                    ...
                    [
                        ...
                        new TableCellViewModel { Html = x.Role },
                    ...
                    [
                        ...
                        new TableRowActionViewModel { Label = "Khóa", Url = "#", Icon = "bi-trash", CssClass = "btn btn-sm btn-outline-danger", RequiresConfirm = true, ConfirmMessage = "Mock thao tác khóa tài khoản này?" }
                    ]
                }).ToList()
            }
        };
    ...
    public IActionResult Details(int id)
    {
        ...
        var model = new ManagementDetailsPageViewModel
        {
            ...
            Subtitle = "Thông tin phân quyền và triển khai mock auth.",
            Breadcrumbs = Breadcrumbs("Chi tiết tài khoản", "Tài khoản", "/Admin/Accounts"),
            SummaryCards =
            [
                new SummaryCardViewModel { Title = "Vai trò", Value = item.Role, Description = "Điều hướng dashboard theo session", Icon = "bi-person-badge", AccentClass = "primary" },
                new SummaryCardViewModel { Title = "Trạng thái", Value = item.Status, Description = "Badge trạng thái mock", Icon = "bi-shield-check", AccentClass = "success" }
            ],
            Sections =
            [
                new DetailSectionViewModel { Title = "Thông tin đăng nhập", Items = [new() { Label = "Email", Value = item.Email }, new() { Label = "Mật khẩu demo", Value = item.Password }, new() { Label = "Vai trò", Value = item.Role, IsBadge = true, BadgeClass = AppUi.StatusBadgeClass("Đang hoạt động") }] },
            ...
            Timeline =
            [
                new TimelineItemViewModel { Title = "Đăng nhập demo", Meta = "Mock", Description = "Tài khoản có thể được dùng để chuyển hướng role-based dashboard.", AccentClass = "primary" },
                new TimelineItemViewModel { Title = "TODO kết nối DB", Meta = "Backend", Description = "Sẽ map về bảng Account và quyền truy cập thật sau.", AccentClass = "warning" }
            ]
        };
    ...
    private static ManagementFormPageViewModel BuildForm(string title, string cancelUrl, DemoAccount? account = null)
    {
        ...
        {
            Title = title,
            Subtitle = "Form mock chưa ghi xuống database.",
            Breadcrumbs = Breadcrumbs(title, "Tài khoản", "/Admin/Accounts"),
            FormTitle = title,
            FormDescription = "Dùng để demo layout form, validation UI và cấu trúc dữ liệu account.",
            CancelUrl = cancelUrl,
            ...
            [
                new FormSectionViewModel
                {
                    ...
                    [
                        ...
                        new FormFieldViewModel { Label = "Vai trò", Name = "Role", Type = "select", Required = true, Options = [new() { Label = "Admin", Value = "Admin", Selected = account?.Role == "Admin" }, new() { Label = "Staff", Value = "Staff", Selected = account?.Role == "Staff" }, new() { Label = "Teacher", Value = "Teacher", Selected = account?.Role == "Teacher" }] },
